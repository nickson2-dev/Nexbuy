...
const ExperienceCenter: React.FC<ExperienceCenterProps> = ({ products, onAddToCart, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[700] bg-black text-white overflow-hidden flex flex-col font-mono selection:bg-indigo-500">
      ...
      <header className="relative z-10 flex items-center justify-between p-8 border-b border-white/10 backdrop-blur-md">
        <div className="flex items-center gap-6">
          <div className="w-12 h-12 bg-indigo-600 rounded-xl flex items-center justify-center font-black italic shadow-[0_0_30px_rgba(79,70,229,0.5)]">NB</div>
          <div>
            <h2 className="text-xl font-black tracking-tighter uppercase italic">Nexbuy Research Labs</h2>
            <p className="text-[10px] text-indigo-400 font-bold tracking-[0.3em] uppercase">Core Simulation Protocol v2.5</p>
          </div>
        </div>
        <button 
          onClick={onClose}
          className="px-8 py-3 bg-white/5 border border-white/20 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-red-500/20 hover:border-red-500/50 transition-all flex items-center gap-2"
        >
          Terminate Session
        </button>
      </header>
        ...
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="space-y-10 animate-slide-up">
            ...
            <div className="space-y-2">
              <p className="text-indigo-500 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4 max-w-md">
               {Object.entries(current.specs || {}).map(([key, val]) => (
                 <div key={key} className="p-5 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 relative overflow-hidden group">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 pt-6">
              <button 
                ...
                className="w-full sm:w-auto bg-white text-black px-12 h-20 rounded-2xl font-black text-lg flex items-center justify-center gap-4 hover:scale-105 active:scale-95 transition-all shadow-[0_0_50px_rgba(255,255,255,0.2)]"
              >
              ...
              <div className="flex flex-col">
                 ...
                 <span className="text-3xl font-black tabular-nums">+{current.xpGain || 100} XP</span>
              </div>
            </div>
          </div>

          <div className="relative group perspective-1000 hidden lg:block">
            <div className="relative w-full aspect-square bg-slate-900/40 rounded-[64px] border border-white/10 backdrop-blur-md p-24 flex items-center justify-center overflow-hidden">
               {isScanning && (
      ...
      <footer className="relative z-10 p-12 flex justify-between items-center border-t border-white/5 backdrop-blur-md">
        <div className="flex gap-6">
          <button 
            onClick={prev}
            className="w-20 h-20 rounded-2xl border border-white/10 flex items-center justify-center hover:bg-white/5 hover:border-indigo-500/50 transition-all group"
          >
        ...
        <div className="flex flex-col items-end gap-3">
           <div className="flex gap-3">
            {products.map((_, i) => (
              <div 
                key={i} 
                className={`h-1.5 rounded-full transition-all duration-700 ${i === index % products.length ? 'w-24 bg-indigo-500' : 'w-4 bg-white/10'}`}
              ></div>
            ))}
          </div>
